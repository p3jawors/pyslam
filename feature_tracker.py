...
class DescriptorFeatureTracker(FeatureTracker):
    ...
    # out: keypoints and descriptors
    def detectAndCompute(self, frame, mask=None):
        # print('FEATURE_TRACKER  detectAndComput| tracker FRAME: ', frame)
        # print('FEATURE_TRACKER  detectAndComput| tracker mask: ', mask)
        return self.feature_manager.detectAndCompute(frame, mask)


    # out: FeatureTrackingResult()
    def track(self, image_ref, image_cur, kps_ref, des_ref):
        # print('HOW THE FUCK AM I IN HERE?')
        kps_cur, des_cur = self.detectAndCompute(image_cur)
        # convert from list of keypoints to an array of points
        kps_cur = np.array([x.pt for x in kps_cur], dtype=np.float32)

        # print('before getting idxs_ref shapes|')
        # print('des ref shape: ', des_ref.shape)
        # print('des_cur shape: ', des_cur.shape)
        # print('kps_ref shape: ', kps_ref.shape)
        # print('kps_cur shape: ', kps_cur.shape)
        ...
        # print('all ref points: ', res.kps_ref.shape)
        res.kps_cur = kps_cur  # all the current keypoints
        # print('all curr points: ', res.kps_cur.shape)
        res.des_cur = des_cur  # all the current descriptors
        # print('all desc: ', res.des_cur.shape)

        res.kps_ref_matched = np.asarray(kps_ref[idxs_ref]) # the matched ref kps
        # print('matched kp: ', res.kps_ref_matched.shape)
        res.idxs_ref = np.asarray(idxs_ref)

        # print('(all cur pts) kps_cur: ', np.asarray(kps_cur).shape)
        # print('idxs_cur: ', np.asarray(idxs_cur).shape)
        # print(kps_cur)
        # print(idxs_cur)
        # print('MAX IDX: ', max(idxs_cur))
        # print('LEN KPS: ', len(kps_cur))
