...
# RfnetFeature = import_from('feature_rfnet', 'RfnetFeature')
from feature_rfnet import RfnetFeature
# print('DISK FEATURE TYPE: ', type(DiskFeature2D))
# print(DiskFeature2D)
# print('RFNET FEATURE TYPE: ', type(RfnetFeature))
# print(RfnetFeature)
...
class FeatureManager(object):
    def __init__(self, num_features=kNumFeatureDefault,
        ...
        # init detector
        # --------------------------------------------- #
        # print('CUSTOM!!!!!!! feature detector type: ', self.detector_type)
        if self.detector_type == FeatureDetectorTypes.SIFT or self.detector_type == FeatureDetectorTypes.ROOT_SIFT:
            ...
            self.num_levels = 1 # force
            self.need_color_image = True
            # print('CUSTOM2!!!! : Rfnet Class type: ', RfnetFeature)
            self._feature_detector = RfnetFeature(num_features=self.num_features)
            #
            #
        else:
        ...
        if self.is_detector_equal_to_descriptor:
            ...
            # print('detector: ', self._feature_detector)
            # print('descriptor: ', self._feature_descriptor)
        else:
            ...
            if self.descriptor_type == FeatureDescriptorTypes.SIFT or self.descriptor_type == FeatureDescriptorTypes.ROOT_SIFT:
                ...
                if self.detector_type != FeatureDetectorTypes.RFNET:
                    raise ValueError("You cannot use RFNET internal descriptor without RFNET detector!\nPlease, select RFNET as both descriptor and detector!")
                # print('USING RFNET DETECTOR AND DESCRIPTOR')
                # print('detector: ', self._feature_detector)
                self._feature_descriptor = self._feature_detector  # reuse detector object
                # print('descriptor: ', self._feature_descriptor)
    ...
    def detectAndCompute(self, frame, mask=None, filter = True):
        ...
        if self.use_pyramid_adaptor:
            ...
            if self.is_detector_equal_to_descriptor:
                ...
                # print('manager FRAME: ', frame)
                # print('manager MASK: ', mask)
                print(self._feature_detector)
