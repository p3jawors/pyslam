...
class RfnetFeature:
    ...
    #     print('\n\nFINALLY IN THE RIGHT FUCKING SPOT!!!!')
    #     raise Exception
    ###############################################################################
    # detect and compute
    ###############################################################################
    ...
    def detectAndCompute(self, curr_img, mask):
        ...
        if self.ref_img is None:
            ...
            self.kp_ref, self.des_ref, self.ref_img = self.model.detectAndCompute(
                    curr_img, self.device, curr_img.shape)
            ...
            self.kp_curr, self.des_curr, curr_img = self.model.detectAndCompute(curr_img, self.device, curr_img.shape)
            ...
            img1, img2 = reverse_img(curr_img), reverse_img(self.ref_img)
            # keypoints1 = list(map(to_cv2_kp, mkp1))
            # keypoints2 = list(map(to_cv2_kp, mkp2))
            keypoints1 = list(map(to_cv2_kp, self.kp_ref))
            keypoints2 = list(map(to_cv2_kp, self.kp_curr))
            ...
            returned_kp = self.kp_curr.cpu().numpy()
            ...
            self.ref_img = curr_img
            # return returned_kp, returned_des
