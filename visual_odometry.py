...
class VisualOdometry(object):
    def __init__(self, cam, groundtruth, feature_tracker):
        ...
        # print('FEATURE TRACKET AT INIT: ', self.feature_tracker)
    ...
    def processFirstFrame(self):
        ...
        # print('RUNNING FIRST DETECT AND COMPUTE')
        self.kps_ref, self.des_ref = self.feature_tracker.detectAndCompute(self.cur_image)
        # print('returned kps_ref: ', type(self.kps_ref))
        # print(np.asarray(self.kps_ref).shape)
        # print(self.kps_ref)
        # convert from list of keypoints to an array of points
        self.kps_ref = np.array([x.pt for x in self.kps_ref], dtype=np.float32)
        # print('converted kps_ref: ', type(self.kps_ref))
        # print(self.kps_ref.shape)
        self.draw_img = self.drawFeatureTracks(self.cur_image)

    def processFrame(self, frame_id):
        # print('RUNNING DETECT AND COMPUTE')
        # track features
        self.timer_feat.start()
        # print('\n\nfeature tracker.track: ', self.feature_tracker.track)
        # print('feature tracker.detectAndCompute: ', self.feature_tracker.detectAndCompute)
        # print('feature tracker: ', self.feature_tracker)
        # print('before feature track func::::')
        # print('kps_ref: ', type(self.kps_ref))
        # print('kps_ref shape: ', self.kps_ref.shape)
        # print('des_ref: ', type(self.des_ref))
        # print('des_ref shape: ', self.des_ref.shape)
        ...
        if (self.feature_tracker.tracker_type == FeatureTrackerTypes.LK) and (self.kps_ref.shape[0] < self.feature_tracker.num_features):
            ...
            # print('kps_cur', np.asarray(self.kps_cur).shape)
            # print('des_cur', self.des_cur)
